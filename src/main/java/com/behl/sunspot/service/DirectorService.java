...
public class DirectorService {
	...
	public Director retrieve(final String directorId) {
		DocumentSnapshot retrievedDirector;
		try {
			retrievedDirector = firestore.collection(Entity.DIRECTOR.getName()).document(directorId).get().get();
		} catch (InterruptedException | ExecutionException e) {
			log.error("Unable to retrieve director with id {}: {}", directorId, e);
	...
	public List<DirectorDto> retrieveAll() {
		try {
	...
	public void delete(final String directorId) {
		try {
			firestore.collection(Entity.DIRECTOR_MOVIE_MAPPING.getName()).whereEqualTo("directorId", directorId).get()
					.get().getDocuments().forEach((QueryDocumentSnapshot directorMovieMapping) -> {
